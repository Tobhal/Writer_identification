...
from Util import h_from_image, least, greatest
...
image_a = img
print(image_a.shape, np.max(image_a))
b1 = image_a
    ...
    for j in range(h9, h2, 1):
        if image_a[i][j] == 0:
            ...
            if image_a[i][j] == 1:
                ...
                if image_a[i][j + 1] == 1 and image_a[i - 1][j + 1] == 1 and image_a[i - 1][j] == 1 and image_a[i - 1][
                    j - 1] == 1 and image_a[i][j - 1] == 1 and image_a[i + 1][j - 1] == 1 and image_a[i + 1][j] == 1 and \
                        image_a[i + 1][j + 1] == 1:
                    b1[i][j] = 1
                else:
                    b1[i][j] = 3
...
class MyTestCase(unittest.TestCase):
    def test_something(self):
        global app, h1, h2, h3, h4

        self.assertEqual(h1, 55)
        self.assertEqual(h2, 1954)
        self.assertEqual(h3, 14)
        self.assertEqual(h4, 1353)
